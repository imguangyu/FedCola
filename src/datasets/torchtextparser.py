...
def fetch_torchtext_dataset(args, dataset_name, root, tokenizer, seq_len, num_embeddings):
    ...
    if os.path.exists(os.path.join(root, f'tr_{seq_len}.npz')) and os.path.exists(os.path.join(root, f'te_{seq_len}.npz')):
        with np.load(os.path.join(root, f'tr_{seq_len}.npz'), allow_pickle=True) as tr, np.load(os.path.join(root, f'te_{seq_len}.npz'), allow_pickle=True) as te:
            tr_inputs, tr_targets = tr['arr_0'], tr['arr_1']
            te_inputs, te_targets = te['arr_0'], te['arr_1']
    else:
        if tokenizer is None:
    ...
    raw_train.name = dataset_name
    raw_test.task = 'cls'
    raw_test.modality = 'txt'
    raw_train.name = dataset_name
