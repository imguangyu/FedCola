...
def batchwise_cdist(samples1, samples2, eps=1e-6):
    ...
    if len(samples1.size()) != 3 or len(samples2.size()) != 3:
        ...
        # print(f'samples1: {samples1.shape}, samples2: {samples2.shape},')
...
class MCSoftContrastiveLoss(nn.Module):
    ...
    def __init__(self, init_shift=15, init_negative_scale=15, num_samples=7, uniform_lambda=0, vib_beta=0, reduction='sum'):
        super().__init__()
        if reduction not in {'mean', 'sum', None}:
            raise ValueError('unknown reduction {}'.format(reduction))
        self.reduction = reduction

        shift = init_shift * torch.ones(1, device="cuda:0" if torch.cuda.is_available() else "cpu")
        negative_scale = init_negative_scale * torch.ones(1, device="cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    def _compute_loss(self, input1, input2):
        ...
        self.shift.data = self.shift.to(distance.device)
        self.negative_scale.data = self.negative_scale.to(distance.device)
    ...
    def forward(self, image_features, caption_features, image_logsigma=0, caption_logsigma=0, **kwargs):
        uniform_loss = 0
        uniform_loss_val = 0
        vib_loss = 0
        vib_loss_val = 0
        ...
        if self.vib_beta != 0:
            # print('image_features', image_features.shape, image_logsigma, 'caption_features', caption_features.shape,
            #       caption_logsigma)
            vib_loss = \
                self.kl_divergence(image_features.mean(dim=1), image_logsigma) + self.kl_divergence(
                    caption_features.mean(dim=1), caption_logsigma)
